...
def test_run_sample_qc_surfaces_zero_library_size_sample():
    # QC should report a dead/empty sample rather than choke on it — it's
    # compute_cpm's job (downstream) to refuse to normalize it.
    df = pd.DataFrame({
        "sample_A": [10, 20, 30],
        "sample_B": [0, 0, 0],
        "sample_C": [12, 18, 33],
    ...
    assert result.loc["sample_B", "library_size"] == 0
    assert result.loc["sample_B", "genes_detected"] == 0
