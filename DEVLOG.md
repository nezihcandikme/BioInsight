...
That consistency is the actual finding here, more than either number on its own: "zero false alarms, finds roughly a tenth of what count-based tools find" isn't a quirk of one dataset's noise structure or one lab's sequencing depth — it held up, within a couple of percentage points, on a second species with a completely different design. That's a meaningfully stronger claim to make about the welch method's behavior than one dataset alone could support. (`--method moderated` on `pasilla` hasn't been run yet — same command as `airway`'s, `python benchmarks/compare_results.py --dataset pasilla --method moderated`, whenever that's worth doing.)
